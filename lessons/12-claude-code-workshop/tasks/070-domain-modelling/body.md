...
A reference brief — a small book catalog: a **book** has a title, author, ISBN, and description,
can be marked **available** or **checked out**, and (once you reach the reviews feature) can
receive multiple **reviews** — is a good stand-in if you don't yet have your own product brief.
Getting concepts and relationships precise now means the schema, validation, and UI all line up
later. Ambiguity is a context hazard: synonyms and vague terms lead the agent to merge things that
should stay distinct.

## Steps

1. Give the agent your brief and ask it to **restate the domain in its own words** — the core
   entities, how they relate, and the lifecycle of a book's availability status (or your domain's
   equivalent).
